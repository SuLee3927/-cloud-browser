...
| | 🏠 家庭模式 | 🖥️ 服务器模式 |
|---|---|---|
| 浏览器跑在哪 | 你家/公司的真实电脑 | 服务器自己 |
| IP / 指纹 | **真实住宅 IP + 真实浏览器指纹** | 机房 IP（地域看你的服务器在哪） |
| 要常开电脑吗 | 要 | **不要** |
| 要隧道吗 | 要（本仓库 `windows/`） | 不要 |
| 适合 | 风控敏感的站点、长期登录态 | 只要某地域的 IP、纯抓取/定时任务 |

服务器模式见 [`server-only/README.md`](server-only/README.md)。
